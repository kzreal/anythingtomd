...
@mcp.tool(name="convert_file", description="🔄 转换文件为 Markdown。支持 Word/Excel/PDF/图片，可指定输出格式")
def convert_file(file_path: str, output_format: str = "markdown", max_level: str = "0") -> str:
    """
    将文件转换为 Markdown 格式。
    ...
    Args:
        file_path: 文件路径
        output_format: 输出格式 (markdown|json|sections)
    ...
    """
    try:
        result = _convert_file_internal(file_path, output_format, max_level)
...
def _convert_file_internal(file_path: str, output_format: str = 'markdown',
                       max_level: Union[str, int] = 0) -> Union[str, Dict]:
    """
    转换文件为指定格式（内部实现）
    """
    path = Path(file_path)
    ...
    logger.info(f"开始转换文件: {file_path}, 类型: {file_type}, 输出格式: {output_format}")
...
# 导出内部函数供测试使用
__all__ = ['analyze_file_internal', 'convert_file_internal', 'convert_batch_internal', 'list_sections_internal']
...
def convert_file_internal(file_path: str, output_format: str = 'markdown',
                       max_level: Union[str, int] = 0) -> Union[str, Dict]:
    """转换文件（供测试使用）"""
    return _convert_file_internal(file_path, output_format, max_level)
